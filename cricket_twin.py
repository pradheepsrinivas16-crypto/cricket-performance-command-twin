...
# ==========================================
GEMINI_API_KEY = "AQ.Ab8RN6IFeugYwa8CADaJFzaSR50_qWjFRQ51tExLTMo9OGHLTg"

client = None
api_ready = False
if GEMINI_API_KEY and not GEMINI_API_KEY.startswith("PASTE_YOUR"):
    try:
...
# ==========================================
# 🤖 LOCAL OLLAMA SERVER ENGINE
# ==========================================
def query_local_ollama(prompt, model_name="llama3"):
    ...
    try:
        ...
        if response.status_code == 200:
            return response.json().get("response", "No analytics returned.")
        return f"Ollama Core Error Code: {response.status_code}"
    except Exception as e:
        return f"⚠️ Network Shield Active: {str(e)[:120]}. Check local engine status."
...
with tab2:
    ...
    if st.button("🤖 Run Strategic Recommendation Inference"):
        if st.session_state.balls_simulated == 0:
            ...
            with st.spinner("Processing tactical field vectors via local engine..."):
                ollama_prompt = f"""
...
# ==========================================
# OVERHAULED MODULE 3: BIOMECHANICAL SUITE (STRENGTHS / WEAKNESSES)
# ==========================================
with tab3:
...
# ==========================================
# OVERHAULED MODULE 4: ATHLETE BASE & RECOVERY MATRIX
# ==========================================
with tab4:
